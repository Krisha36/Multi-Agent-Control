...
class LaunchFileGenerator:
    ...
    def generate_launch_file(self):
        ...
        with open(self.output_file_name, "w") as f:
            ...
            f.write('    <arg name="world_name" value="$(find turtlebot3_gazebo)/worlds/empty.world"/>\n')
            ...
            for k in range(self.num_robots):
                ...
                #f.write('        <arg name="use_tf_static" default="false"/>\n')
                f.write('        <param name="robot_description" command="$(find xacro)/xacro --inorder $(find turtlebot3_description)/urdf/turtlebot3_$(arg model).urdf.xacro" />\n')
                #f.write(f'       <param name="tf_prefix" value="tb3_{k}" />\n')
                #f.write(f'            <node name="joint_state_publisher" pkg="joint_state_publisher" type="joint_state_publisher"> \n')
                #f.write(f'            <param name="use_gui" value="False"/> \n' )
                #f.write('             </node>\n')
                f.write('        <node pkg="robot_state_publisher" type="robot_state_publisher" name="robot_state_publisher" output="screen">\n')
                #f.write('         <node pkg="robot_state_publisher" type="robot_state_publisher" name="robot_state_publisher"\n')
                f.write('            <param name="publish_frequency" type="double" value="50.0" />\n')
                #f.write(f'             <param name="use_tf_static" value="$(arg use_tf_static)"/>\n')
                f.write('        </node>\n')
                f.write(f'        <param name="tf_prefix" value="tb3_{k}" />\n')
                
                f.write(f'        <node name="spawn_urdf" pkg="gazebo_ros" type="spawn_model" args="-urdf -model tb3_{k} -x $(arg robot_{k}_pos_x) -y $(arg robot_{k}_pos_y) -z $(arg robot_{k}_pos_z) -Y $(arg robot_{k}_yaw) -param robot_description" output="screen" />\n')
                f.write('    </group>\n')
